...
ViewSetList = Union[ModelViewSetType, Sequence[ModelViewSetType]]
...
def merge_sets(*sets) -> list:
    return [ route for set in sets for route in set ]


def get_names_and_model_from_step(step) -> str:
    """
...
def build_nested_url(path: ViewSetList) -> Tuple[str, str]:
    """
    ...
    for i, step in enumerate(path):
        ...
        singular, plural, model = get_names_and_model_from_step(step)
        converter = CONVERTERS_MAP.get(type(model._meta.pk), 'int')

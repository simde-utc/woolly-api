...
class OrderValidator:
	...
	def _check_order(self):
		...
		user_prev_ongoing_orders = self.user.orders.filter(status=OrderStatus.ONGOING.value, sale=self.sale.pk).exclude(pk=self.order.pk)
	...
	def _check_quantities(self):
		...
		for item in order_qt.per_item:
			...
			if is_quantity(item.max_per_user) and user_qt.per_item.get(item, 0) + order_qt.per_item[item] > item.max_per_user:
				self._add_error("Vous ne pouvez pas prendre plus de " + str(item.max_per_user) + " " + str(item.name) + " par utilisateur.")
		...
		for group in order_qt.per_group:
			...
			if is_quantity(group.max_per_user) and user_qt.per_group.get(group, 0) + order_qt.per_group[group] > group.max_per_user:
				self._add_error("Vous ne pouvez pas prendre plus de " + str(group.max_per_user) + " " + str(group.name) + " par utilisateur.")

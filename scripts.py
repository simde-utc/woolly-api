...
def full_process(sale_pk: int=None, save: bool=True, backup_after: bool=False):
	dump_sale_excel(sale_pk)
	update_orders(sale_pk, save)
	verify_orderlines(sale_pk, save)
	gen_tickets(sale_pk, save)
	if backup_after:
		dump_sale_excel(sale_pk)
	...
	def _proccess_order(order):
		...
		for orderline in order.orderlines.all():
			for orderlineitem in orderline.orderlineitems.all():
				...
				data.update({
					'UUID': str(orderlineitem.id).replace('-', ''), # Remove dashes for wap
...
def update_orders(sale_pk: int=None, save: bool=True):
	"""
	...
	print("Updated {} orders.".format(len(results)))

def gen_tickets(sale_pk: int=None, save: bool=True):
...
def verify_orderlines(sale_pk: int=None, save: bool=True):
